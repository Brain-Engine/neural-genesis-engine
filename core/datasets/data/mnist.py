...
def train_dataset(data_dir, transform=MNISTTrainTransform, **kwargs):
    return datasets.MNIST(root=data_dir, train=True, transform=transform, download=True, **kwargs)


# val dataset example for tiny-image-net
def val_dataset(data_dir, transform=MNISTValidationTransform, **kwargs):
    return datasets.MNIST(root=data_dir, train=False, transform=transform, download=True, **kwargs)
